...
def main(
    ...
    global posToken,negToken
    _, posToken, negToken = tokenizer.encode(['Yes','No'])
    ...
    def printInputOutput(s, output, inputs,prompt):
        print("prompt 0:",prompt[0])
        print("prompt 0 wordl lenght:",len(prompt[0].strip().split(' ')))
        print("output 0", output[0])
        print("output 0 word lenght:",len(output[0].strip().split(' ')))
        print("s[1] shape",s[1].shape)
        print("input_ids length",inputs['input_ids'][0].shape)
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        scores = generation_output.scores[0].softmax(dim=-1)  # only look at the first output. 
        logits = torch.tensor(scores[:,[posToken, negToken]], dtype=torch.float32).softmax(dim=-1)
        input_ids = inputs["input_ids"].to(device)
        L = input_ids.shape[1]
        s = generation_output.sequences
        
        output = tokenizer.batch_decode(s, skip_special_tokens=True)
        # printInputOutput(s,output, inputs,prompt)
    ...
    with open(test_data_path, 'r') as f:
        ...
        for i, batch in tqdm(enumerate(zip(batch(instructions,batch_size), batch(inputs,batch_size)))):
